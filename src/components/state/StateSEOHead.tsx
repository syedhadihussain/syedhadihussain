...
const StateSEOHead = ({ state, countryCode, title, description, keywords }: StateSEOHeadProps) => {
  ...
  const baseUrl = "https://seohadi.com";
  const canonicalUrl = `${baseUrl}/${language}/${countryCode}/${state.code}/`;

  // Generate structured data for the state page
  const localBusinessSchema = {
    "@context": "https://schema.org",
    "@type": "LocalBusiness",
    "@id": `${baseUrl}/#localbusiness-${state.code}`,
  ...
  const breadcrumbSchema = {
    ...
    "itemListElement": [
      ...
      },
      {
        ...
        "position": 2,
        "name": "United States",
        "item": `${baseUrl}/${language}/${countryCode}/`
      },
      {
        "@type": "ListItem",
        "position": 3,
    ...
    <Helmet>
      ...
      {SUPPORTED_LANGUAGES.map((lang) => (
        <link
          ...
          rel="alternate"
          hrefLang={lang}
          href={`${baseUrl}/${lang}/${countryCode}/${state.code}/`}
        />
      ))}
      <link rel="alternate" hrefLang="x-default" href={`${baseUrl}/en/${countryCode}/${state.code}/`} />
